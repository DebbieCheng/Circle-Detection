...
namespace OtsuThreshold
{
    class Otsu
    {
        ...
        public int getOtsuThreshold_Array(int[] hist)
        {
            byte t = 0;
            float[] vet = new float[256];
            //int[] hist = new int[256];
        ...
        public Bitmap GetBinaryImg(Bitmap img, int threshold)
        {
            ...
            for (int i = 0; i < img.Width; i++)
            {
                ...
                {
                    tmpvalue = (tmp.GetPixel(i, j).R + tmp.GetPixel(i, j).G + tmp.GetPixel(i, j).B) / 3;
        ...
        public Bitmap GetnonBackImg(Bitmap img, int threshold)
        {
            int width = img.Width;
            ...
            for (int i = 0; i < width; i++)
            {
                ...
                {
                    if ((img.GetPixel(i, j).R + img.GetPixel(i, j).G + img.GetPixel(i, j).B) / 3 <= threshold)
                    {
